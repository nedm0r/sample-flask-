...
image_name = f"nedm0r/docker-flask-project:{next_version}"
client.images.build(path=".", tag=image_name, rm=True, pull=True)
print(f"Successfully built image: {image_name}")

# Push the image with the specified tag
client.images.push(repository="nedm0r/docker-flask-project", tag=next_version)
print(f"Successfully pushed image: {image_name}")

# Push the image with the 'latest' tag
latest_tag = "latest"
latest_image_name = f"nedm0r/docker-flask-project:{latest_tag}"
image_to_tag = client.images.get(image_name)
image_to_tag.tag(repository="nedm0r/docker-flask-project", tag=latest_tag)
client.images.push(repository="nedm0r/docker-flask-project", tag=latest_tag)
